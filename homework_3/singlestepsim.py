...
def get_middle_node_xy(nv,q0):
    #nv must be odd
    mid_ind = int((nv-1)/2)
    return np.array([q0[2*mid_ind], q0[2*mid_ind+1]])

def simulate_beam(u0, q0, control_input,dt,nv,RodLength, draw_figure=True):
    ndof = 2 * nv
    ...
    fixed_index = np.array([0, 1, 2*(nv-2), 2*(nv-2)+1, 2*(nv-1), 2*(nv-1)+1]) # Fixed DOFs

    # Free index
    free_index = np.setdiff1d(all_DOFs, fixed_index) # All the DOFs are free except the fixed ones
    
    q0[2*(nv-1)]     = control_input[0]
    q0[2*(nv-1)+1]   = control_input[1]
    q0[2*(nv-2)]     = control_input[0] - deltaL*np.cos(control_input[2])
    q0[2*(nv-2)+1]   = control_input[1] - deltaL*np.sin(control_input[2])
